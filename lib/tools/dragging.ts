...
function onPointerDown(this: DraggableLive2DModel, e: FederatedPointerEvent) {
    this.dragging = true;
    this._pointerX = e.global.x - this.x;
    this._pointerY = e.global.y - this.y;
}

function onPointerMove(this: DraggableLive2DModel, e: FederatedPointerEvent) {
    if (this.dragging) {
        this.position.x = e.global.x - this._pointerX;
        this.position.y = e.global.y - this._pointerY;
    }
}

function onPointerUp(this: DraggableLive2DModel, e: FederatedPointerEvent) {
    this.dragging = false;
}
